...
class TarjetaCredito(IPersistencia):
    ...
    def crear_tarjeta_por_consola(self):
        ...
        propietario = input(
            "Introduce el nombre completo del propietario de la tarjeta: "
        )
    ...
    def diccionario_a_objeto(self, diccionario_tarjeta: dict):
        try:
            if (
                "Numero de cuenta" in diccionario_tarjeta
                and diccionario_tarjeta["Numero de cuenta"]
            ):
            ...
            if (
                "Fecha registro" in diccionario_tarjeta
                and diccionario_tarjeta["Fecha registro"]
            ):
                fecha_registro = Fecha()
                fecha_registro.diccionario_a_objeto(
                    diccionario_tarjeta["Fecha registro"]
                )
                self.set_fecha_registro(fecha_registro)

            if (
                "Fecha caducidad" in diccionario_tarjeta
                and diccionario_tarjeta["Fecha caducidad"]
            ):
                fecha_caducidad = Fecha()
                fecha_caducidad.diccionario_a_objeto(
                    diccionario_tarjeta["Fecha caducidad"]
                )
                self.set_fecha_caducidad(fecha_caducidad)

            if (
                "Propietario" in diccionario_tarjeta
                and diccionario_tarjeta["Propietario"]
            ):
