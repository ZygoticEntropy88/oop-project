...
class Cancion(IPersistencia):
    ...
    def solicitar_usuario_por_consola(self, motivo_consulta=""):
        cancion_id: str = input(
            f"Introduzca el ID de youtube de la canción {motivo_consulta}: "
        )
        self.set_identificador(cancion_id)

        if (
            input("Le gustaría guardar más datos de la canción? (s/n): ")
            .lower()
            .strip()
            in ["s", "si", "y", "yes", "1"]
        ):
            cancion_nombre = input(
                "Introduzca el título de la canción (Presione enter para dejar en blanco): "
            )
            if cancion_nombre != "":
                self.set_nombre(cancion_nombre)
            cancion_artista = input(
                "Introduzca el artista de la canción (Presione enter para dejar en blanco): "
            )
            if cancion_artista != "":
                self.set_artista(cancion_artista)
            cancion_genero = input(
                "Introduzca el género de la canción (Presione enter para dejar en blanco): "
            )
            if cancion_genero != "":
