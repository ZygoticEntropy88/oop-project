...
class TarjetaCredito(IPersistencia):
    ...
    def diccionario_a_objeto(self, diccionario_tarjeta: dict):
        try:
            ...
            if (
                "Fecha de registro" in diccionario_tarjeta
                and diccionario_tarjeta["Fecha de registro"]
            ):
                self.set_fecha_registro(diccionario_tarjeta["Fecha de registro"])

            if (
                "Fecha de caducidad" in diccionario_tarjeta
                and diccionario_tarjeta["Fecha de caducidad"]
            ):
                self.set_fecha_caducidad(diccionario_tarjeta["Fecha de caducidad"])

            if (
                "Nombre del titular" in diccionario_tarjeta
                and diccionario_tarjeta["Nombre del titular"]
            ):
